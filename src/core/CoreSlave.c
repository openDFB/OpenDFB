...
)
{
    ...
    switch (CoreDFB_CallMode( core_dfb )) {
        ...
        case COREDFB_CALL_INDIRECT: {
			ICoreSlaveRequestor requestor;
			ICoreSlave *super = (ICoreSlave *)&requestor;
			ICoreSlaveRequestorInit( &requestor, core_dfb, obj );


            Core_PushCalling();
            ret = super->GetData( super, address, bytes, ret_data );
            Core_PopCalling();
...
)
{
    ...
    switch (CoreDFB_CallMode( core_dfb )) {
        ...
        case COREDFB_CALL_INDIRECT: {
            ...
            Core_PushCalling();
            ret = super->PutData( super, address, bytes, data );
            Core_PopCalling();
...
DFBResult
ICoreSlaveRequestor_GetData(ICoreSlave *thiz,
                    void*                                      address,
                    u32                                        bytes,
                    u8                                        *ret_data
)
{
    ...
    CoreSlaveGetData       *args = (CoreSlaveGetData*) args_alloc( args_static, sizeof(CoreSlaveGetData) );
    CoreSlaveGetDataReturn *return_args;

    if (!args)
        return (DFBResult) D_OOM();

    return_args = (CoreSlaveGetDataReturn*) args_alloc( return_args_static, sizeof(CoreSlaveGetDataReturn) + bytes * sizeof(u8) );
    ...
    D_DEBUG_AT( DirectFB_CoreSlave, "ICoreSlaveRequestor_%s()\n", __FUNCTION__ );


    args->address = address;
    args->bytes = bytes;

    ret = (DFBResult) CoreSlave_Call( thiz->obj, FCEF_NONE, CoreSlaveCall_GetData, args, sizeof(CoreSlaveGetData), return_args, sizeof(CoreSlaveGetDataReturn) + bytes * sizeof(u8), NULL );
    if (ret) {
...
DFBResult
ICoreSlaveRequestor_PutData(ICoreSlave *thiz,
                    void*                                      address,
                    u32                                        bytes,
                    const u8                                  *data
)
{
    ...
    char        args_static[FLUXED_ARGS_BYTES];
    char        return_args_static[FLUXED_ARGS_BYTES];
    CoreSlavePutData       *args = (CoreSlavePutData*) args_alloc( args_static, sizeof(CoreSlavePutData) + bytes * sizeof(u8) );
    CoreSlavePutDataReturn *return_args;

    if (!args)
        return (DFBResult) D_OOM();

    return_args = (CoreSlavePutDataReturn*) args_alloc( return_args_static, sizeof(CoreSlavePutDataReturn) );
    ...
    ret = (DFBResult) CoreSlave_Call( thiz->obj, FCEF_NONE, CoreSlaveCall_PutData, args, sizeof(CoreSlavePutData) + bytes * sizeof(u8), return_args, sizeof(CoreSlavePutDataReturn), NULL );
    if (ret) {
...
void ICoreSlaveRequestorInit(ICoreSlaveRequestor *thiz, CoreDFB *core, CoreSlave *obj)
{
    ...
    thiz->base.GetData= ICoreSlaveRequestor_GetData;
    thiz->base.PutData = ICoreSlaveRequestor_PutData;
}
                                ...
                                unsigned int *ret_length )
{
	...
	ICoreSlaveReal real;
	ICoreSlave *super = (ICoreSlave *)&real;
	ICoreSlaveRealInit( &real, core_dfb, obj );

    switch (method) {
        case CoreSlaveCall_GetData: {
            D_UNUSED
            ...
            return_args->result = super->GetData( super, args->address, args->bytes, (u8*) ((char*)(return_args + 1)) );
        ...
        case CoreSlaveCall_PutData: {
            ...
            return_args->result = super->PutData( super, args->address, args->bytes, (u8*) ((char*)(args + 1)) );
            if (return_args->result == DFB_OK) {
            }
