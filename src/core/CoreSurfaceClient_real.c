...
DFBResult
ISurfaceClientReal_FrameAck(ISurfaceClient *thiz,
                        u32                                   flip_count
                        )
{
     int                i;
     ...
     D_DEBUG_AT( DirectFB_CoreSurfaceClient, "ISurfaceClient_Real::%s( count %u ) <- old count %u\n",
                 __FUNCTION__, flip_count, thiz->base.obj->flip_count );

     surface = thiz->obj->surface;
     ...
     thiz->obj->flip_count = flip_count;
...
void ISurfaceClientRealInit(ISurfaceClientReal *thiz, CoreDFB *core, CoreSurfaceClient *obj)
{
	...
	thiz->base.FrameAck = ISurfaceClientReal_FrameAck;
}
