 ...
 */
typedef enum {
    CorePaletteCall_SetEntries = 1,
    CorePaletteCall_SetEntriesYUV = 2,
} CorePaletteCall;
...
typedef struct _IPalette IPalette;
typedef struct _IPaletteReal IPaletteReal;
typedef struct _IPaletteRequestor IPaletteRequestor;

struct _IPalette{
	CoreDFB *core;
    CorePalette *obj;

	DFBResult (*SetEntries)(IPalette *thiz, const DFBColor *colors,u32 num,u32 offset);
	DFBResult (*SetEntriesYUV)(IPalette *thiz, const DFBColorYUV *colors, u32 num, u32 offset);
};

struct _IPaletteReal{
	IPalette base;
};

void IPaletteRealInit(IPaletteReal *thiz, CoreDFB *core, CorePalette *obj);

struct _IPaletteRequestor{
	IPalette base;
};
