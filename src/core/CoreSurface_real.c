...
DFBResult
ISurfaceReal_SetConfig(ISurface *thiz,
                        const CoreSurfaceConfig                   *config
                        )
{
     D_DEBUG_AT( DirectFB_CoreSurface, "ISurfaceReal_%s()\n", __FUNCTION__ );

     D_ASSERT( config != NULL );

     return dfb_surface_reconfig( thiz->obj, config );
}


DFBResult
ISurfaceReal_Flip(ISurface *thiz,
                   bool                                       swap
                   )
{
     DFBResult ret;
     ...
     dfb_surface_lock( thiz->obj );

     ret = dfb_surface_flip( thiz->obj, swap );

     dfb_surface_unlock( thiz->obj );
...
DFBResult
ISurfaceReal_GetPalette(ISurface *thiz,
                         ...
                         )
{
     ...
     ret = (DFBResult) dfb_palette_ref( thiz->obj->palette );
     if (ret)
          return ret;

     *ret_palette = thiz->obj->palette;
...
DFBResult
ISurfaceReal_SetPalette(ISurface *thiz,
                         CorePalette                               *palette
                         )
{
     D_DEBUG_AT( DirectFB_CoreSurface, "ISurfaceReal_%s()\n", __FUNCTION__ );
     ...
     return dfb_surface_set_palette( thiz->obj, palette );
}


DFBResult
ISurfaceReal_SetAlphaRamp(ISurface *thiz,
                         u8                                         a0,
                         u8                                         a1,
                         u8                                         a2,
                         u8                                         a3
                         )
{
     ...
     return dfb_surface_set_alpha_ramp( thiz->obj, a0, a1, a2, a3 );
}


DFBResult
ISurfaceReal_SetField(ISurface *thiz,
                         s32                                        field
                         )
{
     D_DEBUG_AT( DirectFB_CoreSurface, "ISurfaceReal_%s()\n", __FUNCTION__ );

     return dfb_surface_set_field( thiz->obj, field );
}
...
DFBResult
ISurfaceReal_PreLockBuffer(ISurface *thiz,
...
DFBResult
ISurfaceReal_PreLockBuffer2(ISurface *thiz,
                         ...
                         )
{
     ...
     D_DEBUG_AT( DirectFB_CoreSurface, "ISurfaceReal_%s( surface %p, role %d, eye %d, accessor 0x%02x, access 0x%02x, %slock )\n",
...
DFBResult
ISurfaceReal_PreReadBuffer(ISurface *thiz,
                         CoreSurfaceBuffer                         *buffer,
...
DFBResult
ISurfaceReal_PreWriteBuffer(ISurface *thiz,
                         CoreSurfaceBuffer                         *buffer,
                         const DFBRectangle                        *rect,
                         CoreSurfaceAllocation                    **ret_allocation
                         )
...
DFBResult
ISurfaceReal_PreLockBuffer3(ISurface *thiz,
                         CoreSurfaceBufferRole                      role,
                         u32                                        flip_count,
                         DFBSurfaceStereoEye                        eye,
                         CoreSurfaceAccessorID                      accessor,
                         ...
                         )
{
     ...
     D_DEBUG_AT( DirectFB_CoreSurface, "ISurfaceReal_%s( surface %p, role %d, count %u, eye %d, accessor 0x%02x, access 0x%02x, %slock )\n",
...
DFBResult
ISurfaceReal_CreateClient(ISurface *thiz,
                         ...
                         )
{
     ...
     return dfb_surface_client_create( thiz->core, thiz->obj, ret_client );
}


DFBResult
ISurfaceReal_BackToFrontCopy(ISurface *thiz,
                   DFBSurfaceStereoEye                                 eye,
                   ...
                   )
{
     ...
     dfb_back_to_front_copy_stereo( thiz->obj, eye, left_region, right_region, 0 );
...
void ISurfaceRealInit(ISurfaceReal *thiz, CoreDFB *core, CoreSurface *obj)
{
	...
	thiz->base.SetConfig = ISurfaceReal_SetConfig;
	thiz->base.Flip = ISurfaceReal_Flip;
	thiz->base.GetPalette = ISurfaceReal_GetPalette;
	thiz->base.SetPalette = ISurfaceReal_SetPalette;
	thiz->base.SetAlphaRamp = ISurfaceReal_SetAlphaRamp;
	thiz->base.SetField = ISurfaceReal_SetField;
	thiz->base.PreLockBuffer = ISurfaceReal_PreLockBuffer;
	thiz->base.PreLockBuffer2 = ISurfaceReal_PreLockBuffer2;
	thiz->base.PreReadBuffer = ISurfaceReal_PreReadBuffer;
	thiz->base.PreWriteBuffer = ISurfaceReal_PreWriteBuffer;
	thiz->base.PreLockBuffer3 = ISurfaceReal_PreLockBuffer3;
	thiz->base.CreateClient = ISurfaceReal_CreateClient;
	thiz->base.BackToFrontCopy = ISurfaceReal_BackToFrontCopy;
}
